...
class ECGDataset(Dataset):
    def __init__(
        self,
        root="./",
        metadata_dir=f"/ssd-shared/mimiciv-ecg-echonotes/physionet.org/files/mimic-iv-ecg/1.0/processed/machine_measurements_abnormality_v3.csv",
        npy_file_dir="",
        split="Train",
        from_numpy=True,
        ecg_transform=None,
        ...
        self.ecg2 = [None] * len(self.meta_data)
    ...
    def get_data(self):
        for idx in tqdm(range(len(self))):
            ...
            if self.from_numpy:
                ...
                self.ecg2[idx] = np.load(
                    # f"{self.meta_data.iloc[idx]['segment_path']}{self.meta_data.iloc[idx]['segment_path'][-9:-1]}_ECG_II.npy"
                    f"/ssd-shared/mimiciv-ecg-echonotes/physionet.org/files/mimic-iv-ecg/1.0/{self.meta_data.iloc[idx]['path']}_ECG_III.npy"
                )
            ...
            if not np.isfinite(self.ecg1[idx]).all():
                print("here1")
            if not np.isfinite(self.ecg2[idx]).all():
                print("here2")
            #     continue
    ...
    def __getitem__(self, index):
        ...
        ecg2 = signal.resample(self.ecg2[index], 1000).reshape(1, 1000)

        # ecg1 = self.ecg1[index].reshape(1, 5000)
        # ecg2 = self.ecg1[index].reshape(1, 5000)
        ...
        return (ecg1, ecg2, abnormality_label)
